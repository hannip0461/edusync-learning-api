...
보호자 조회 인증은 먼저 토큰을 한 보호자 주체에 연결한다. 경로의 `guardianId`가 해당 주체와 일치하고 `guardian_links`가 학습자 조회를 허용해야 진행 상태를 반환한다. Classic ASP 어댑터는 별도의 읽기 전용 연동 경계로 유지하며, 소스와 JSON 응답은 계약 테스트로 확인한다. 어댑터에는 사용자 인증이 없고 경계가 호출자 주소이므로, `REMOTE_ADDR` 검사를 배치 스크립트가 아니라 ASP 소스 안에 둔다.

## 쓰기 경로의 인가 강도

두 쓰기 경로의 인가 강도가 다르다. 학습자 Bearer 토큰은 학습자 한 명에 묶이고 본문의 `learner_id`가 다르면 403이다. 플레이어 HMAC 자격 증명은 발행자 하나를 지목하고 학습자 식별자를 담지 않으므로 소유권을 검사하지 않는다. 두 경로 모두 학습자와 강의 존재, `ACTIVE` 수강, 수강 기간 안의 `occurred_at`은 요구한다. 근거와 기각한 대안은 DECISIONS D10에 있다.
